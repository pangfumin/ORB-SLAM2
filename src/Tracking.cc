...
const int forceWidth = 800,
	forceHeight = 600;
const int OFFSET4SCALE=10;


extern volatile bool stopChild;
	...
	mpLocalMapper(NULL)
{
    ...
    imageTopic = (string)fSettings["Camera.topic"];
    isCompressed = (int)fSettings["Camera.compressed"];
    useExternalLocalisation = (int)fSettings["ExternalLocalization"];
    ...
    cout << "- fps: " << fps << endl;
    cout << "- Image Topic: " << imageTopic << " " << (isCompressed ? "(compressed)" : "(uncompressed)") << endl;
...
void Tracking::SetLoopClosing(LoopClosing *pLoopClosing)
{
    ...
    KeyFrame::loopCloser = pLoopClosing;
}
...
void Tracking::RunFromROS()
{
    ...
    else th = image_transport::TransportHints ("raw");
    image_transport::Subscriber subscribe = it.subscribe (imageTopic, 1, &Tracking::grabImageFromROS, this, th);
...
void Tracking::RunFromVideoFile(const string &videoInputFile, int seekSecond)
{
	cv::VideoCapture vcap (videoInputFile);
	if (!vcap.isOpened())
		return;

	double fps = vcap.get(CV_CAP_PROP_FPS);
	vcap.set (CV_CAP_PROP_POS_MSEC, seekSecond*1000.0);
	ros::Rate rate (fps);
	...
	while (true) {
		...
		if (imgFrame.channels() != 1) {
			if (mbRGB)
				cvtColor (imgFrame, imgFrame, CV_RGB2GRAY);
			else
				cvtColor (imgFrame, imgFrame, CV_BGR2GRAY);
		}

		if (imgFrame.cols > forceWidth) {
			cv::resize (imgFrame, imgFrame, cv::Size(forceWidth, forceHeight));
		}

		frameId++;
		// send to process
		if (localizationOnly==true)
			ProcessImageLocalizer (imgFrame, ros::Time::now().toSec());
		else
			ProcessImage (imgFrame, ros::Time::now().toSec());
...
void Tracking::grabImageFromROS (const sensor_msgs::ImageConstPtr &msg)
{
    cv::Mat im;
...
void Tracking::ProcessImage (cv::Mat &im, double timestamp)
{
    // Start real processing
    if(mState==WORKING || mState==LOST)
        mCurrentFrame = Frame(im,timestamp,mpORBextractor,mpORBVocabulary,mK,mDistCoef);
    else // For initialization and post-map-restoration
    ...
    else
    {
        ...
        if(bOK and mpMap->MapIsReadonly==false)
        {
            mpMapPublisher->SetCurrentCameraPose(mCurrentFrame.mTcw);

            // emulate initial behavior
            if (mState==MAP_OPEN)
...
void Tracking::ProcessImageLocalizer(cv::Mat &img, double timestamp)
{
	mCurrentFrame = Frame (img, timestamp, mpIniORBextractor, mpORBVocabulary, mK, mDistCoef);
	mLastProcessedState=mState;

	bool bOK = Relocalisation();
	if (bOK) {
		mState = WORKING;
		cout << "Relocalisation succesful" << endl;
	}
	else {
		mState = LOST;
		cout << "We're lost" << endl;
	}
	mLastFrame = Frame (mCurrentFrame);
	mpFramePublisher->Update(this);
	if (!mCurrentFrame.mTcw.empty()) {
...
{
    // Tracking from previous frame or relocalisation was succesful and we have an estimation
...
bool Tracking::Relocalisation2 ()
{
	mCurrentFrame.ComputeBoW ();

	vector<KeyFrame*> vpCandidateKFs = mpKeyFrameDB->DetectRelocalisationCandidates (&mCurrentFrame);
...
volatile bool __debugMatching = false;
...
bool Tracking::Relocalisation()
{
    ...
    // I Disabled relocalisation against last keyframe, because it may be unreliable
    // So, it will always try to relocalise against all possible keyframes
//    if(!RelocalisationRequested() or mState==MAP_OPEN) {
...
//            if(nmatches<15)
    ...
    while(nCandidates>0 && !bMatch)
    {
        for(size_t i=0; i<vpCandidateKFs.size(); i++)
        {
            ...
            // this piece is meant to run under debugger
            if (__debugMatching==true) {
            	const string dbgPoseName ("debug/pose.png");
            	string debugdir = "debug";
            	vpCandidateKFs[i]->debug(debugdir);
            	mCurrentFrame.debug(debugdir);
            	cv::Mat dbgImg = pSolver->debugPose (dbgPoseName);
            }
...
//    KeyFrame *kfOfftime = mpMap->offsetKeyframe(kfNear, (float)DEFAULT_OFFSET_TIME);
    ...
    cv::Mat kfNearPos = kfNear->GetCameraCenter();
    double offDistO = cv::norm(kfNearPos - kp->GetCameraCenter());
    double offDistE = cv::norm(kfNear->extPosition - kp->extPosition);
