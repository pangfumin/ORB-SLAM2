    ...
    mnIterations(0), mnBestInliers(0), N(0)
{
    ...
    _imWidth = F.im.cols, _imHeight = F.im.rows;
}
...
volatile bool _doDebugPose = false;
...
inline void drawRect (cv::Mat &canvas, const cv::Point2f &center, const float length, const cv::Scalar &color)
{
	const float r = length/2;
	cv::Point2f p1 (center.x-r, center.y-r);
	cv::Point2f p2 (center.x+r, center.y+r);
	cv::rectangle (canvas, p1, p2, color);
}


cv::Mat PnPsolver::debugPose (const std::string &filename)
{
	cv::Mat dbgImg = cv::Mat::ones(_imHeight, _imWidth, CV_8UC3);

	for (int i=0; i<mvP3Dw.size(); i++) {
		...
		double ue = uc + fu * Xc * invZc;
		double ve = vc + fv * Yc * invZc;
		cv::Point2f p32d (ue, ve);

		cv::circle (dbgImg, P2D, 2.1, cv::Scalar(255,0,0));
		drawRect (dbgImg, p32d, 6, cv::Scalar(0,0,255));
		cv::line (dbgImg, P2D, p32d, cv::Scalar(0,255,0));
	}

	if (!filename.empty()) {
		cv::imwrite (filename, dbgImg);
		return cv::Mat();
	}
	else return dbgImg;
}
...
cv::Mat PnPsolver::debugKeyMatching(const Frame &F, const vector<MapPoint*> &vpMapPointMatches)
{
	cv::Mat dbgImg = F.im.clone();
	cv::cvtColor(dbgImg, dbgImg, CV_GRAY2BGR);

	for (int i=0; i<vpMapPointMatches.size(); i++) {
		const MapPoint *mp = vpMapPointMatches[i];
		if (!mp) continue;
