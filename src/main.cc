...
//    string strVocFile = ros::package::getPath("ORB_SLAM")+"/"+argv[1];
    ...
    boost::thread trackingThread(&ORB_SLAM::Tracking::RunFromROS,&Tracker);
    ...
    vector<ORB_SLAM::MapPoint*> mpRefs = World.GetReferenceMapPoints();
    vector<ORB_SLAM::MapPoint*> _mpAll = World.GetAllMapPoints();
    set<ORB_SLAM::MapPoint*> mpAll = set<ORB_SLAM::MapPoint*>(_mpAll.begin(), _mpAll.end());
    for (int i=0; i<mpRefs.size(); i++) {
    	ORB_SLAM::MapPoint *curPt = mpRefs[i];
    	if (mpAll.count(curPt) != 0)
    		continue;
    	else
    		std::cout << "Map Point Reference #" << curPt->mnId << " not found in MP set!" << std::endl;
    }

    sort(vpKFs.begin(),vpKFs.end(),ORB_SLAM::KeyFrame::lId);
    cout << "Map size: " << vpKFs.size() << endl;
