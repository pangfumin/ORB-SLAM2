...
LoopClosing * KeyFrame::loopCloser = NULL;
...
const string keyframeDebugImage ("keyframe.jpg");


void KeyFrame::debug (const string &dirname)
{
	if (!this->im.empty()) {
		const string kfDbg = dirname + "/" + keyframeDebugImage;
		cv::Mat dbgImage = this->im.clone();
		...
		cv::imwrite (kfDbg, dbgImage);
	}

//	const string kfDebug = dirname + "/keyframe.log";
//	fstream kffd (kfDebug.c_str(), fstream::out|fstream::trunc);
//	kffd << "Descriptor" << endl;
//	kffd << this->mDescriptors;
//	kffd.close();
}
